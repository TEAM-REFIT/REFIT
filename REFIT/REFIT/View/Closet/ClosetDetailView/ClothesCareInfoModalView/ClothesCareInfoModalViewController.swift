...
class ClothesCareInfoModalViewController: UIViewController {
    ...
    let cotton = [
        "면은 사계절 내내 입을 수 있으며, 쉽게 찢어지지 않아 오래 입을 수 있습니다.\n그러나 쉽게 구겨지고, 열에 약하며 물에 오래 담궈두었을 때 탈색의 위험이 있으니 주의해야 합니다.",
        "중성세제로 세탁해 주세요.\n미온수에서 손 세탁을  가장 추천해요.\n세탁기 사용 시 울코스에서 30~40°C로 세탁해 주세요.",
        "세탁기로 탈수 시, 시간은 5분 이내로 하는 것이 좋아요.\n열에 약하므로 직사광선을 피해 통풍이 잘 되는 곳에 건조해 주세요.\n젖은 면 옷을 옷걸이에 걸어 건조하면 형태의 변형이 생길 수 있습니다.\n건조대를 이용해 건조하세요.",
    ...
    // top
    let tShirtAndHood = [
        "데일리로 가장 흔하게 입는 옷 입니다.\n자주 입는 만큼 잘 헤져 관리가 필요해요!",
    ...
    let shirt = [
        "깔끔하게 입고 싶을 때 유용하게 입힙니다.\n그러나 오염이나 구김이 가지 않게 잘 관리해줘야 합니다.",
    ...
    let knit = [
        "신축성이 좋고, 보온력도 좋아 가을, 겨울, 봄에 자주 입힙니다.\n그러나 형태가 변하기 쉬워 주의가 필요합니다.",
    ...
    // pants
    let dressPants = [
        "정장바지는 차려입을 때, 슬랙스는 데일리템으로 많이 활용됩니다.\n깔끔한 핏이 특징인 만큼 구김이 지거나 주름이 펴지지 않게 주의해야 합니다.",
    ...
    let jeans = [
        "청바지는 어디에나 잘 어울리는 아이템입니다.\n세탁/관리 시 물이 빠지거나 천이 늘어나지 않게 주의하세요!",
    ...
    let skirt = [
        "치마는 형태나 소재가 다양해 관리 방법이 가지각색입니다.\n세탁 전 의류의 케어라벨을 확인하세요.",
    ...
    let cottonPant = [
        "면바지는 자주 활용되는 아이템 중 하나입니다.\n내구성이 좋다는 특징이 있지만, 구김이 가거나 색이 빠지지 않게 조심하세요!",
    ...
    ///outer
    let padding = [
        "패딩은 겨울 철, 빼 놓을 수 없는 아이템입니다.\n세탁시 솜이 뭉치지 않게 주의해야 합니다.",
    ...
    let coat = [
        "가을에 자주 활용되는 아이템입니다.\n'only드라이클리닝'인 코트가 많아 세탁 시 주의를 기울여야 하나, 어렵지 않으니 걱정하지 않으셔도 됩니다.",
        "드라이클리닝을 권장합니다.\n손세탁 시에는 홈드라이클리닝세제를 물에 풀어 20분 정도 담가둔 후 손세탁하세요.",
        "손세탁 후 세탁기에 넣어 탈수합니다.\n건조대에 눕혀 건조하세요.",
        "두꺼운 정장용 옷걸이에 걸어 보관하세요.\n단추를 채우고 보관하세요."
    ]
    let jacket = [
        "봄, 가을에 자주 활용되는 아이템입니다.\n아우터 중에서는 세탁도 쉬운 편 입니다.",
    ...
    // shoes
    let canvas = [
        "신발은 손세탁하는 것이 가장 좋으나, 세탁법이 까다로우니, 번거롭다면 전문 신발 빨래방에 맡기는 것이 좋습니다.",
        "표백성분이 없는 연성세제(중성세제)를 이용해 오염이 있는 부분을 부드럽게 문질러주세요.",
        "바람이 잘 통하는 곳에 뒤집어 말려주시고, 건조 후에는 신문지를 말아 넣어주세요.",
        ""
    ]
    
    let leatherShoes = [
        "가죽 신발은 쉽게 상할 수 있어 주의해야 합니다.\n그러나 오염이 생겼을 때 바로 닦아주기만 하면 걱정 없습니다!",
        "가죽틀리너나 마른 수건으로 가볍게 닦아 오염을 제거하세요.",
        "",
        "열에 약하니 서늘한 곳에 보관하세요.\n안 쓰는 영양크림으로 닦아주어도 좋아요."
    ]
    
    let sportsShoes = [
        "러닝화나 스포츠화는 땀이나 오염물질에 노출될 확률이 높아 주의가 필요합니다.\n그러나 세탁 시에 형태가 변할 수 있으니 평소에 잘 관리 해 주는 것을 추천합니다. ",
        "형태가 변형될 수 있으니 세탁보다는 오염 부위를 수건으로 닦거나 솔로 문질러 가볍게 털어주세요.\n세탁을 해야 한다면 미지근한 물에 연성세제를 이용해 솔로 닦아주세요.",
        "젖었을 때는 마른 수건으로 두드린 후 그늘에 말려주세요.",
        "서늘하고 건조한 곳에 보관해주세요."
    ]
    
    let suedeShoes = [
        "운동화에 자주 사용되는 스웨이드 재질은 관리 시 전용 세제가 필요합니다.",
        "오염이 생겼을 때는 빠른 시일 내로 오염을 제거하는 것이 좋습니다.\n신발용품을 판매하는 곳에서 스웨이드 클리너를 구매해 사용하세요.\n스웨이드 클리너를 이용해 오염부위를 살살 문질러 오염을 제거하세요.",
        "젖었을 때는 마른 수건으로 두드린 후 그늘에 말려주세요.",
        "신발이 겹치지 않게 신발자엥 놓아 보관하세요."
    ]
    // etc
    let onePiece = [
        "원피스는 형태나 소재가 다양해 관리 방법이 가지각색입니다.\n세탁 전 의류의 케어라벨을 확인하세요.",
        "일반 원피스는 중성세제를 이용해 세탁하는 것이 좋습니다.\n악세사리가 부착되어 있다면 드라이크리닝을 맡기는 것이 좋습니다.",
        "옷걸이에 걸어 건조하지 말고 수건을 이용해 물기 제거 후 건조대에 널어 그늘에서 말려주세요.",
        "다림질 후 옷걸이에 걸어 보관하세요"
    ]
    let clothHat = [
        "볼캡부터 버킷햇 등 다양한 형태의 천 모자들은 관리가 어렵지 않으니, 오래 쓰기 위해서는 땀이나 이물질이 묻었다면 바로 세탁해 주는 것이 좋습니다.",
        "미지근한 물에서 솔이나 칫솔로 닦아 세탁하세요.",
        "그늘지고 바람이 잘 통하는 곳에서 건조하세요. ",
        "신문지로 모자 속을 채워두면 세탁 후에도 형태가 변하지 않습니다."
    ]
    let beanie = [
        "겨울철에 자주 쓰이는 털 모자를 관리할 때는 형태가 변하지 않게 주의를 기울여야 합니다.",
    ...
    let bag = [
        "가방은 자주 쓰지만, 세탁하기는 번거롭습니다.\n종류나 소재도 다양해 관리하기도 까다롭습니다.\n다음 관리방법은 면 소재 백팩을 기준으로 작성되었으니, 다른 가방들은 소재별 관리방법을 확인해 주세요!",
